...
function generartabla(objeto) {
    ...
    user.forEach(usuarios => {
        ...
        <td><button onclick="javascript:habilSus('${usuarios.id}','${usuarios.estado_cuenta}')" style="background-color: greed;">${usuarios.estado_cuenta}</button></td></tr>`;
    });
    return tabla += '</table>';
}
...
function habilSus(id, estado) {
    ...
    fetch(`../../api/usuarios/estadoCuenta.php?id=${id}&estado=${estadoActual}`)
        .then(Response => Response.json())
        .then(data => {
            console.log(data);
            if (data.success) {
